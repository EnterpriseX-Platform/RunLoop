        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <div className="relative">
            ...
            >
              ...
              {/* Dev-only hint pointing at the seed log. We deliberately do
                  NOT render plaintext credentials in the UI — the seed
                  script prints a random password on first boot and the
                  operator captures it from there. Shown only in dev. */}
              {process.env.NEXT_PUBLIC_SHOW_DEV_HINT === 'true' && (
                ...
                >
                  <div style={{ color: 'var(--t-text-secondary)' }}>
                    <span style={{ opacity: 0.6 }}>{'//'}</span> dev seed
                  </div>
                  <div>
                    <span style={{ opacity: 0.6 }}>{'>'}</span>{' '}
                    Run <code>npm run db:seed</code> and copy the printed password
                  </div>
                </div>
              )}
            </div>
